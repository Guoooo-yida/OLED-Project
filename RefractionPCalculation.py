...
def Calculate_RP(thickLayer, refractIndex, angleRad, waveLength):  # u_0是入射角度，在这个函数中是一个数字

    numLayer = len(thickLayer)
    if numLayer == 1:
        if angleRad == 0:
            return Calculate_RP(thickLayer, refractIndex, 1e-14, waveLength)
        else:
            ...
            rp1 = (refractIndex[1] ** 2 * (mu1 + 1j * mv1) - refractIndex[0] ** 2 * (mu2 + 1j * mv2)) / (
                    refractIndex[1] ** 2 * (mu1 + 1j * mv1) + refractIndex[0] ** 2 * (mu2 + 1j * mv2))
            rp2 = (refractIndex[2] ** 2 * (mu2 + 1j * mv2) - refractIndex[1] ** 2 * (mu3 + 1j * mv3)) / (
                    refractIndex[2] ** 2 * (mu2 + 1j * mv2) + refractIndex[1] ** 2 * (mu3 + 1j * mv3))

            rp = (rp1 + rp2 * cmath.exp(2 * 1j * thickLayer[0] * 2 * cmath.pi * (mu2 + 1j * mv2) / waveLength)) / (
        ...
        if angleRad == 0:
            ...
            rp1 = (refractIndex[1] ** 2 * (mu1 + 1j * mv1) - refractIndex[0] ** 2 * (mu2 + 1j * mv2)) / (
                    refractIndex[1] ** 2 * (mu1 + 1j * mv1) + refractIndex[0] ** 2 * (mu2 + 1j * mv2))
            rp = (rp1 + Calculate_RP(thickLayer[1:], refractIndex[1:], u_1, waveLength) * cmath.exp(
                2 * 1j * thickLayer[0] * 2 * cmath.pi * (mu2 + 1j * mv2) / waveLength)) / (
                1 + rp1 * Calculate_RP(thickLayer[1:], refractIndex[1:], u_1, waveLength) * cmath.exp(
                2 * 1j * thickLayer[0] * 2 * cmath.pi * (mu2 + 1j * mv2) / waveLength))
